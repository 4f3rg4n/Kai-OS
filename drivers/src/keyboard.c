...
//global kernel buffer
key_event keyboard_buf[KEYBOARD_BUF_SIZE] = {0};
char* keys_buf[KEYBOARD_BUF_SIZE] = {0};
...
void keyboard_init() {
    ...
    irq_set_handler(keyboard_irq, keyboard_handler);
    buf_cls();
...
void keyboard_handler() {
    ...
    if(is_small_letter(symbols[0][code]) || is_number(symbols[0][code]) || is_special_char(symbols[0][code])) {
        keyboard_buf[buf_ctr].scancode = code; // // get key scancode and store the key event
        keyboard_buf[buf_ctr].data = m_keyboard;
        putch(key_to_ascii(keyboard_buf[buf_ctr]));
        buf_ctr = (buf_ctr + 1) % KEYBOARD_BUF_SIZE;
    }
}
...
u32bit get_buf(char* buf) {
    u32bit ctr = 0;
    while (key_to_ascii(get_last_event()) != '\n'){}
    for(; ctr != KEYBOARD_BUF_SIZE && key_to_ascii(keyboard_buf[ctr]) != '\n'; ctr++){
        buf[ctr] = key_to_ascii(keyboard_buf[ctr]);
    }
    buf_cls();
...
key_event get_last_event() {
    return keyboard_buf[((buf_ctr > 0) ? buf_ctr : KEYBOARD_BUF_SIZE) - 1];
}

char get_char() {
    char ch;
    while (!get_last_event().scancode){}
    ch = key_to_ascii(get_last_event());
    if(buf_ctr > 0)
        buf_ctr--;
    return ch;
}